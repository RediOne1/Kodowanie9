...
public class Kodowanie9 {

    static double p;
    static byte[] arr;
    static ArrayList<String> mse, snr;

    public static void main(String[] args) throws IOException {
        File file = new File("mg.tga");
        File file2 = new File("test.r");
        File file3 = new File("test.tga");

        kodowanie(file, file2, 7);
        dekodowanie(file2, file3);
        mse(file, file3);

    }

    private static void kodowanie(File in, File out, int k) throws IOException {
    ...
    private static void dekodowanie(File in, File out) throws IOException {
        try (OutputStream output = new BufferedOutputStream(new FileOutputStream(out))) {
    ...
    private static void mse(File in, File out) throws IOException {
        mse = new ArrayList<>();
        snr = new ArrayList<>();
        byte[] b2 = b_to_arr(out), b1 = b_to_arr(in);
        for (int i = 0; i < 4; i++)
            mse(b2, b1, i);
        mse.forEach(System.out::println);
        System.out.println();

        snr.forEach(System.out::println);

    }

    private static byte[] b_to_arr(File file) throws IOException {
    ...
    private static void mse(byte[] b1, byte[] b2, int x) throws IOException {
        ...
        if (x != 3) {
            ...
            mse.add("mse(" + color + ")= " + mset);
            snr.add("snr(" + color + ")= " + (signal / noise) + " (" + 10 * log10(signal / noise) + "dB)");
        } else {
            mset = noise / ((b1.length - 44));
            mse.add("mse = " + mset);
            snr.add("snr = " + (signal / noise) + "(" + 10 * log10(signal / noise) + "dB)");
        }
    }
